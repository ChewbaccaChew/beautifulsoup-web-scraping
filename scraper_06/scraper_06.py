...
def get_all_pages():
    ...
    # # запрос на страницу сайта
    # r = requests.get(url="https://shop.casio.ru/catalog/g-shock/filter/gender-is-male/apply/", headers=headers)
    #
    # if not os.path.exists("data"):  # если указанный путь не существует, то:
    #     os.mkdir("data")  # создаём директорию
    #
    # # сохраним результат запроса в файл
    # with open("data/page_1.html", "w", encoding="utf-8") as file:  # задаём имя файла
    #     file.write(r.text)  # и записываем в него результат
